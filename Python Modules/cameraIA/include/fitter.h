...
class fitter: public baseObject{
    public:
        ...
        editor editFunctions;
        ///Functions
        //1D ESTIMATES(GETS A,B MU, AND SIGAMA) CHOOSE PROJECTION AND WHETHER TO USE MOMENTS OR FWHM
        std::vector<double> get1DEstimates(const std::vector<double>& projection, const cameraStructs::EstMethod& method);
        // COMPARE 1D MU AND SIGMA VALUES OF TWO ESTIMATES
        double compare1DEstimates(const double & mu1,const double & sigma1,
                                  const double & mu2,const double & sigma2);
        //FIT 1D GAUSSIAN
        std::vector<double> fit1DGaussianToProjection(const cameraStructs::cameraOfflineIAObject& ID,
                                                      const cameraStructs::projAxis& axis,
                                                      const std::vector<double>& estimates);
        //GENERAL 1D GUASSAIN FIT
        std::vector<double> fitGuassian(const std::vector<double> data,
                                        const std::vector<double> estimates);
        //USE 1D FITS TO DO A BVN FIT
        std::vector<double> fitBVN(const cameraStructs::cameraOfflineIAObject& ID,
        ...
        //CALC SIGMA
        double calculateSigmaSquared(const std::vector<double> &data,
                                     const std::vector<double> &position,
                                     const double &mu);
        ...
        double rSquaredOf1DProjection(const cameraStructs::cameraOfflineIAObject& ID,
