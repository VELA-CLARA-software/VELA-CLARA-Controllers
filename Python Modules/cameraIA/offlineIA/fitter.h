...
class fitter: public baseObject{
    public:
        ...
        //FIT 1D GAUSSIAN
        std::vector<double> fit1DGaussianToProjection(const cameraOfflineIAObject& ID,const projAxis& axis, const std::vector<double>& estimates);
        ...
        std::vector<double> fitBVN(const cameraOfflineIAObject& ID, const std::vector<double>& xEstimates, const std::vector<double>& yEstimates);
        //DIRECTLY CALCULATE THE MOMENT OF THE IMAGE DISTRIBUTION
        std::vector<double> covarianceValues(const cameraOfflineIAObject& ID,const double & cut);
        //CALC MU
        ...
        double rSquaredOf1DProjection(const cameraOfflineIAObject& ID, const std::vector<double>& parameters, const projAxis& axis);
