...
laserConfigReader::laserConfigReader( const std::string & configFileLocation1,
...
laserConfigReader::~laserConfigReader(){}
//______________________________________________________________________________
bool laserConfigReader::readConfig()
{
...
//______________________________________________________________________________
void laserConfigReader::addToPVMonitorMapV1( const  std::vector<std::string> &keyVal  )
{
...
//______________________________________________________________________________
void laserConfigReader::addToPVCommandMapV1( const  std::vector<std::string> &keyVal  )
{
...
//______________________________________________________________________________
void laserConfigReader::addPVStruct( std::vector< laserStructs::pvStruct > & pvStruct_v,  const  std::vector<std::string> &keyVal )
{
    /// http://stackoverflow.com/questions/5914422/proper-way-to-initialize-c-structs
    /// init structs 'correctly'
//    pvStruct_v.push_back( laserStructs::pvStruct() );
//    pvStruct_v.back().pvSuffix = keyVal[1];
//    if( keyVal[0] == UTL::PV_SUFFIX_ON  )
//        pvStruct_v.back().pvType = laserStructs::PIL_SHUTTER_PV_TYPE::On;
//    else if( keyVal[0] == UTL::PV_SUFFIX_OFF  )
//        pvStruct_v.back().pvType = laserStructs::PIL_SHUTTER_PV_TYPE::Off;
//    else if( keyVal[0] == UTL::PV_SUFFIX_STA  )
//        pvStruct_v.back().pvType = laserStructs::PIL_SHUTTER_PV_TYPE::Sta;
//
//    debugMessage("Added ", pvComStructs.back().pvSuffix, " suffix" );
}
//______________________________________________________________________________
void laserConfigReader::addTolaserObjectsV1( const std::vector<std::string> &keyVal   )
{
//    if( keyVal[0] == UTL::NAME )
//    {
//        /// http://stackoverflow.com/questions/5914422/proper-way-to-initialize-c-structs
//        /// init structs 'correctly'
//        laserStructs::laserObject shob = laserStructs::laserObject();
//        shob.name = keyVal[ 1 ];
