...
void gunModConfigReader::addToPVStruct(std::vector<rfModStructs::pvStruct>& pvStruct_v, const std::vector<std::string>& keyVal)
{
    if(stringIsSubString(keyVal[0], "SUFFIX"))
    {
        ...
        else if(keyVal[0] == UTL::PV_SUFFIX_GUN_MOD_MAIN_STATE_READ)
        {
        ...
        else if(keyVal[0] == UTL::PV_SUFFIX_GUN_MOD_ILOCK2)
        {
            pvStruct_v.back().pvType = rfModStructs::GUN_MOD_PV_TYPE::ILOCK2_STR;
        }
        else if(keyVal[0] == UTL::PV_SUFFIX_GUN_MOD_ILOCK3)
        {
            pvStruct_v.back().pvType = rfModStructs::GUN_MOD_PV_TYPE::ILOCK4_STR;
        }
        else if(keyVal[0] == UTL::PV_SUFFIX_GUN_MOD_ILOCK4)
        {
            pvStruct_v.back().pvType = rfModStructs::GUN_MOD_PV_TYPE::ILOCK4_STR;
        }
        else if(keyVal[0] == UTL::PV_SUFFIX_GUN_MOD_ILOCK5)
        {
            pvStruct_v.back().pvType = rfModStructs::GUN_MOD_PV_TYPE::ILOCK5_STR;
        }
        ...
        {
            message("!!CONFIG FILE ERROR!! couldn't find ",keyVal[0]," ",keyVal[1]);
        }
