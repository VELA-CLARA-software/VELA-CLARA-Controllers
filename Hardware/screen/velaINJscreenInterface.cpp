...
velaINJscreenInterface::velaINJscreenInterface( const std::string & conf1,
...
velaINJscreenInterface::~velaINJscreenInterface()///This is the destructor for the class
{
    //    debugMessage( "magnetInterface DESTRUCTOR CALLED");
    for (auto && it : screenDualMoveStructsMap )
    {
        debugMessage("in velaINJscreenInterface: delete screenDualMove thread ", it.first );
    ...
    {
        debugMessage("delete velaINJscreenInterface continuousMonitorStructs entry." );
        delete it;
    }
}
//____________________________________________________________________________________
void velaINJscreenInterface::initialise()
{
...
void velaINJscreenInterface::initIsLockedMap()
...
//_______________________________________________________________________________________
bool velaINJscreenInterface::initvelaINJscreenObjects()
{
    return configReader.getvelaINJscreenObjects( allScreentData );
}
//_______________________________________________________________________________________________________________
void velaINJscreenInterface::initScreenChids()
{
...
void velaINJscreenInterface::addChannel( const std::string & pvRoot, screenStructs::pvStruct & pv )
{
...
void velaINJscreenInterface::monitorScreens()
{
    ...
    for( auto && it1 : allScreentData )
    {
        ...
        for( auto && it2 : it1.second.pvMonStructs )
        {
            ...
            ca_create_subscription(it2.second.CHTYPE, it2.second.COUNT, it2.second.CHID,
                                   it2.second.MASK, velaINJscreenInterface::staticEntryScreenMonitor,
...
//_______________________________________________________________________________________________________________
void velaINJscreenInterface::addvelaINJscreenObjectMonitors( screenStructs::pvStruct & pvs,  screenStructs::velaINJscreenObject & obj  )
{
...
//_______________________________________________________________________________________________________________
void velaINJscreenInterface::addScreenDriverMonitors( screenStructs::pvStruct & pvs, screenStructs::screenDriver & obj )// see screenstructs for a screenDriverStatus
{
    continuousMonitorStructsDEV.push_back( new screenStructs::monitorStruct() );
    continuousMonitorStructsDEV.back() -> interface = this;
    continuousMonitorStructsDEV.back() -> monType   = pvs.pvType;
...
//_______________________________________________________________________________________________________________
void velaINJscreenInterface::addScreenDriverStatusMonitors( screenStructs::pvStruct & pvs, screenStructs::screenDriverStatus & obj )// see screenstructs for a screenDriver
{
    continuousMonitorStructsDEV.push_back( new screenStructs::monitorStruct() );
    continuousMonitorStructsDEV.back() -> interface = this;
    continuousMonitorStructsDEV.back() -> monType   = pvs.pvType;
...
//_______________________________________________________________________________________________________________
bool velaINJscreenInterface::isHorizontal( screenStructs::DRIVER_DIRECTION dir )
{
    return dir == screenStructs::DRIVER_DIRECTION::HORIZONTAL;
}
//_______________________________________________________________________________________________________________
bool velaINJscreenInterface::isVertical(  screenStructs::DRIVER_DIRECTION dir )
{
    return dir == screenStructs::DRIVER_DIRECTION::VERTICAL;
}
//_______________________________________________________________________________________________________________
void velaINJscreenInterface::staticEntryScreenMonitor( const event_handler_args args )
{
...
void velaINJscreenInterface::updatePROT03( screenStructs::monitorStruct * ms, const double args )
{
...
//_________________________________________________________________________________________________________________
void velaINJscreenInterface::updatePROT05( screenStructs::monitorStruct * ms, const double args )
{
    // evil evil evil
    screenStructs::screenDriver * obj = reinterpret_cast<screenStructs::screenDriver *> (ms->obj);
    std::stringstream ss;
...
//_________________________________________________________________________________________________________________
void velaINJscreenInterface::updateRPOS( screenStructs::monitorStruct * ms,  const double args )
{
    // evilness (?)
...
//_________________________________________________________________________________________________________________
void velaINJscreenInterface::updateCassettePosition( screenStructs::screenCassette  & cas, const double pos  )
...
//_________________________________________________________________________________________________________________
void velaINJscreenInterface::updatePROT01( screenStructs::monitorStruct * ms, const double args )
{
    screenStructs::screenDriverStatus * obj = reinterpret_cast<screenStructs::screenDriverStatus *> (ms->obj);
    std::stringstream ss;
    ss << obj->parentScreen;
...
//_________________________________________________________________________________________________________________
void velaINJscreenInterface::updateSta( screenStructs::monitorStruct * ms,  const unsigned short args )
...
////_________________________________________________________________________________________________________________
void velaINJscreenInterface::updateScreenState( screenStructs::velaINJscreenObject & scr  )
...
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::is_HandV_CassetteOUT(screenStructs::velaINJscreenObject & scr)
{
    return isHCassetteOUT(scr) && isVCassetteOUT(scr);
}
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::isHCassetteOUT(screenStructs::velaINJscreenObject & scr)
{
    return scr.driver.hCassette.screenState == screenStructs::SCREEN_STATE::H_OUT;
}
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::isVCassetteOUT(screenStructs::velaINJscreenObject & scr)
{
    return scr.driver.vCassette.screenState == screenStructs::SCREEN_STATE::V_OUT;
}
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::entryExists2(const std::string & name, bool weKnowEntryExists )
...
std::vector<bool> velaINJscreenInterface::isScreenIN(const std::vector<std::string> & name)
{
    std::vector<bool> r;
    for( auto && it : name )
...
//___________________________________________________________________________________________________________
std::vector<bool> velaINJscreenInterface::isScreenOUT( const std::vector<std::string> & name )
{
    std::vector<bool> r;
    for( auto && it : name )
...
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::isScreenIN(const std::string & name, bool weKnowEntryExists)
{
...
//___________________________________________________________________________________________________________
screenStructs::SCREEN_STATE velaINJscreenInterface::getScreenState(const std::string & name, const bool weKnowEntryExists)
{
    if( weKnowEntryExists )
...
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::isScreenInState(const std::string & name, screenStructs::SCREEN_STATE sta)
{
    bool r = false;
...
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::screenIN( const std::string & name  )
{   message("velaINJscreenInterface::screenIN(const std::string & name ) called");
    const  std::vector< std::string > names = { name };
    return screenIN( names );
}
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::screenOUT( const std::string & name  )
{   message("velaINJscreenInterface::screenOUT(const std::string & name ) called");
    const  std::vector< std::string > names = { name };
    return screenOUT( names );
}
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::screenIN( const std::vector< std::string > & names  )
{
    std::vector< screenStructs::SCREEN_STATE > v( names.size(), screenStructs::SCREEN_STATE::SCREEN_IN );
    return screenMoveTo( names, v);
}
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::screenOUT( const std::vector< std::string > & names  )
{
...
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::screenMoveTo( const std::string & name, const screenStructs::SCREEN_STATE & states)
{
...
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::screenMoveTo(const std::vector<std::string>& names,const std::vector<screenStructs::SCREEN_STATE>& states  )
{
    ...
    {
        debugMessage("velaINJscreenInterface::screenMoveTo called with following parameters:");
...
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::get_NOT_Locked_and_NOT_MovingScreens(const std::vector<std::string>& scrIN, std::vector<std::string>& scrOUT,
...
//        }
    ...
    {
        message("ABORT! Major Error! in velaINJscreenInterface::get_NOT_Locked_and_NOT_MovingScreens scrOUT.size() != staOUT.size() ",
...
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::sortPnuematicMoverScreen(const std::vector<std::string>& scrIN, std::vector<std::string>& VELA_PNEUMATIC_Scr,
...
//                message( "Can't move ", *itNam, " it is locked");
        ...
        {
            message("ABORT! Major Error! in velaINJscreenInterface::screenMoveTo VELA_HV_MOVER__Sta.size() != VELA_HV_MOVER__Scr.size() ",
...
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::move_VELA_PNEUMATIC_Screens(const std::vector<std::string> & names, const std::vector< screenStructs::SCREEN_STATE > & states  )
...
bool velaINJscreenInterface::send_VELA_PNEUMATIC_Command( const std::vector< chtype* > & CHTYPE, const std::vector< chid* > & CHID, const std::string & m1, const std::string & m2  )
{
    bool ret = false;
...
//______________________________________________________________________________
bool velaINJscreenInterface::move_VELA_HV_MOVER_Screens(const std::vector<std::string> & names, const std::vector< screenStructs::SCREEN_STATE > & states  )
...
//______________________________________________________________________________
void velaINJscreenInterface::staticEntryDualMove(screenStructs::HV_dualMoveStruct & ms )
{
...
//______________________________________________________________________________
void velaINJscreenInterface::killFinishedMoveThreads()
{
...
//______________________________________________________________________________
bool velaINJscreenInterface::setPosition(const std::string & name, const screenStructs::DRIVER_DIRECTION dir, const double value )
{
    bool r = false;
    if( entryExists(allScreentData,name))
    {
...
//______________________________________________________________________________
bool velaINJscreenInterface::set_VELA_HV_MOVER_Position(const std::string& name,  const screenStructs::SCREEN_STATE& sta )
{
    if( is_H_Element( sta) )
    {
...
//______________________________________________________________________________
bool velaINJscreenInterface::set_VELA_HV_MOVER_Position(const std::string& name, const screenStructs::DRIVER_DIRECTION dir, const screenStructs::SCREEN_STATE& sta )
{
    bool r = false;
    double p;
...
//______________________________________________________________________________
bool velaINJscreenInterface::set_VELA_HV_MOVER_Position(const screenStructs::screenDriverStatus& driver, const double value)
{
...
//______________________________________________________________________________
double velaINJscreenInterface::getElementPosition(const screenStructs::screenCassette & scrcas, screenStructs::SCREEN_STATE e )
{
...
//___________________________________________________________________________________________________________
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::is_VELA_PNEUMATIC( const std::string & name )
{
    bool r = false;
    if( entryExists(allScreentData, name ) )
...
//___________________________________________________________________________________________________________
bool velaINJscreenInterface::is_VELA_HV_MOVER ( const std::string & name )
{
    bool r = false;
    if( entryExists(allScreentData, name ) )
...
//___________________________________________________________________________________________________________
const std::vector<std::string> velaINJscreenInterface::get_VELA_PNEUMATIC_Screens(  const std::vector< std::string > & names )
{
...
//___________________________________________________________________________________________________________
const std::vector<std::string> velaINJscreenInterface::get_VELA_HV_MOVER_Screens(  const std::vector< std::string > & names )
{
    std::vector<std::string> r;
    for( auto && it : names )
...
//______________________________________________________________________________
bool velaINJscreenInterface::yagOnV( const screenStructs::screenDriver & scrdr )
{
    return scrdr.vCassette.cassetteElements.at(screenStructs::SCREEN_STATE::V_YAG) == true;
}
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::isVDriveEnabled(const screenStructs::screenDriver & scrdr )
{
    return !isVDriveDisabled(scrdr);
}
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::isVDriveDisabled(const screenStructs::screenDriver & scrdr )
{
    return scrdr.vDriverSTA.state == screenStructs::DRIVER_STATE::DRIVER_DISABLED;
}
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::isHDriveEnabled(const screenStructs::screenDriver & scrdr )
{
    return !isHDriveDisabled(scrdr);
}
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::isHDriveDisabled(const screenStructs::screenDriver & scrdr )
{
    return scrdr.hDriverSTA.state == screenStructs::DRIVER_STATE::DRIVER_DISABLED;
}
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::is_H_Element(const screenStructs::SCREEN_STATE e )
...
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::is_V_Element(const  screenStructs::SCREEN_STATE e )
{   // When moving a cassette element we need to know if it is an H or V element
...
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::is_OUT_AND_VDriveEnabled(const std::string & name)
{
...
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::is_H_element_AND_HDriveEnabled(const screenStructs::SCREEN_STATE e,const screenStructs::screenDriver& scrdr)
{
    return is_H_Element(e) && isHDriveEnabled(scrdr);
}
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::is_V_element_AND_VDriveEnabled(const screenStructs::SCREEN_STATE e,const screenStructs::screenDriver& scrdr)
{
    return is_V_Element(e) && isVDriveEnabled(scrdr);
}
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::is_IN_OR_OUT(const screenStructs::SCREEN_STATE sta )
{
    return sta == screenStructs::SCREEN_STATE::SCREEN_OUT || sta == screenStructs::SCREEN_STATE::SCREEN_IN;
}
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::isScreenOUT(const std::string & name, const bool weKnowEntryExists)
{
    if( entryExists2(name,weKnowEntryExists) )
        return allScreentData.at(name).screenState == screenStructs::SCREEN_STATE::SCREEN_OUT;
    else
...
//_________________________________________________________________________________________________________________
std::vector<bool> velaINJscreenInterface::exists_and_isLocked(const std::string& name)
{
    std::vector<bool> r;
    if( entryExists(allScreentData, name ))
    {
...
//_________________________________________________________________________________________________________________
std::vector<bool> velaINJscreenInterface::exists_and_isNotLocked(const std::string& name)
{
    std::vector<bool> r;
    if( entryExists(allScreentData,name) )
    {
...
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::screen_is_out_AND_sta_is_in(const std::string & name, const screenStructs::SCREEN_STATE sta )
{
    return sta == screenStructs::SCREEN_STATE::SCREEN_IN && isScreenOUT(name);
}
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::screen_is_in_AND_sta_is_out(const std::string & name, const screenStructs::SCREEN_STATE sta )
{
    return sta == screenStructs::SCREEN_STATE::SCREEN_OUT && isScreenIN(name);
}
//___________________________________________________________________________________________________________
void velaINJscreenInterface::update_STA_Bit_map( screenStructs::monitorStruct * ms, const int argsdbr  )
...
//_________________________________________________________________________________________________________________
bool velaINJscreenInterface::isMoving(const std::string & name, const bool weKnowEntryExists)
{
    bool r = false;
    if( entryExists2(name,weKnowEntryExists ) )
...
bool velaINJscreenInterface::isNotMoving(const std::string & name, const bool weKnowEntryExists)
{
    return !isMoving(name);
}
//___________________________________________________________________________________________________________________
std::map< VELA_ENUM::ILOCK_NUMBER, VELA_ENUM::ILOCK_STATE > velaINJscreenInterface::getILockStates( const std::string & name )
{
...
////__________________________________________________________________________________________________________________
std::map< VELA_ENUM::ILOCK_NUMBER, std::string  >  velaINJscreenInterface::getILockStatesStr( const std::string & name )
{
