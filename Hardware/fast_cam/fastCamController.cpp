...
//#endif
//
//    /// There is only ever one fastCamInterface, it's a singleton
//
//    camInterface = &fastCamInterface::getInstance( );
//    std::cout << "The fastCamController got a fastCamInterface instance" << std::endl;
//
...
//______________________________________________________________________________
void fastCamController::attachContext()
{
    camInterface -> attachContext();
}
//______________________________________________________________________________
void fastCamController::startMonitors()
{
    camInterface -> startMonitors();
}
//______________________________________________________________________________
void fastCamController::startMonitors(const std::vector< std::string > & screenNames )
{
...
///
//______________________________________________________________________________
void fastCamController::getObject(const std::string & screenName )
{
...
//______________________________________________________________________________
std::vector< unsigned char > & fastCamController::getRawRef(const std::string & screenName, velaCamStructs::DATA_TYPE type )
{
    getObject( screenName );
    return allCamDataObjects[ screenName ] -> getRawRef( type );
}
//______________________________________________________________________________
std::vector< double > & fastCamController::getProcRef(const std::string & screenName, velaCamStructs::DATA_TYPE type )
{
    getObject( screenName );
    return allCamDataObjects[ screenName ] -> getProcRef( type );
}
//______________________________________________________________________________
unsigned char * fastCamController::getRawRef(const std::string & screenName, velaCamStructs::DATA_TYPE type, size_t part  )
{
    getObject( screenName );
    return allCamDataObjects[ screenName ] -> getRawRef( type, part );
}
//______________________________________________________________________________
double * fastCamController::getProcRef(const std::string & screenName, velaCamStructs::DATA_TYPE type, size_t part  )
{
    getObject( screenName );
    return allCamDataObjects[ screenName ] -> getProcRef( type, part );
}
//______________________________________________________________________________
void fastCamController::resetRaw( const std::string & screen, velaCamStructs::DATA_TYPE type, size_t numShots )
{
    getObject( screen );
...
///
//______________________________________________________________________________
bool fastCamController::getData( const std::string & screen, velaCamStructs::DATA_TYPE type, size_t numShots )
{
    return addData( screen, type, numShots, 0 );
}
//______________________________________________________________________________
bool fastCamController::addData( const std::string & screen, velaCamStructs::DATA_TYPE type, size_t numShots, size_t startShot )
{
    bool ans = false;
    getObject( screen );
#ifdef _OFFLINE_MODE
...
//        std::cout << "while Check aquiringdata? = " <<  acquiringdata << std::endl;
    ...
    if( timeout )
    {
        ...
        std::cout << "TIME OUT IN fastCamController While acquiring Camera Data " << std::endl;
    }
    else
    {
        std::cout << std::endl;
...
//______________________________________________________________________________
bool fastCamController::process( const std::string & screen, velaCamStructs::DATA_TYPE type )
{
    getObject( screen );
    return allCamDataObjects[ screen ] -> process( type );
}
//______________________________________________________________________________
//bool fastCamController::processData( std::string & screen, bool & isDataSaturated )
//{
//    getObject( screen );
...
//______________________________________________________________________________
velaCamStructs::imgCutStruct fastCamController::getIMGCutParam( const std::string & screenName )
{
    getObject( screenName );
    return allCamDataObjects[ screenName ] -> getIMGCutParam( );
}
//______________________________________________________________________________
void fastCamController::setIMGCutParam( const std::string & screenName, velaCamStructs::imgCutStruct & p )
{
    getObject( screenName );
...
///
//______________________________________________________________________________
void fastCamController::serverON( const std::string & screenName   )
{
    //std::cout << "fastCamController::serverON( " << screenName << " )" << std::endl;
    //std::cout << "This feature is disabled at the moment" << std::endl;
    camInterface -> serverON( screenName );
}
//______________________________________________________________________________
void fastCamController::serverON( const char * screenName   )
{
    std::string sn = screenName;
    std::cout << "fastCamController::serverON( " << screenName << " )  .... address === " << camInterface <<  std::endl;
    camInterface -> serverON( sn );
}
//______________________________________________________________________________
void fastCamController::getActiveCameraServers( std::map< std::string, velaCamStructs::CAM_SERVER_STATE > & activeMap )
{
    camInterface -> getActiveCameraServers( activeMap );
}
//______________________________________________________________________________
std::vector< std::string >  fastCamController::getActiveCameraServers(  )
{
