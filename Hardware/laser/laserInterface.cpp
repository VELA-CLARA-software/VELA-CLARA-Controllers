...
// \__,  |  \__/ |  \ /   |__/  |  \__/ |  \
//
laserInterface::laserInterface( const std::string &laserConf,
...
{
//    if( shouldStartEPICs )
//    message("magnet laserInterface shouldStartEPICs is true");
//    else
//    message("magnet laserInterface shouldStartEPICs is false");
    initialise();
}
...
//    debugMessage( "laserInterface DESTRUCTOR COMPLETE ");
}
//______________________________________________________________________________
void laserInterface::killMonitor( laserStructs::monitorStruct * ms )
{
    int status = ca_clear_subscription( ms -> EVID );
    //if( status == ECA_NORMAL)
        //debugMessage( ms->objName, " ", ENUM_TO_STRING(ms->monType), " monitoring = false ");
//    else
        //debugMessage("ERROR laserInterface: in killMonitor: ca_clear_subscription failed for ", ms->objName, " ", ENUM_TO_STRING(ms->monType) );
}
//______________________________________________________________________________
void laserInterface::initialise()
{
    ...
    if( configFileRead )
    {
        message("The laserInterface has read the config file, acquiring objects");
        ...
        if( getDataSuccess )
        {
            ...
            if( shouldStartEPICs )
            {
                message("The laserInterface has acquired objects, connecting to EPICS");
             ...
             message("The laserInterface has acquired objects, NOT connecting to EPICS");
        }
        else
            message( "!!!The laserInterface received an Error while getting magnet data!!!" );
    }
}
//______________________________________________________________________________
bool laserInterface::initObjects()
{
...
//______________________________________________________________________________
void laserInterface::addChannel( const std::string & pvRoot, laserStructs::pvStruct & pv )
{
...
//______________________________________________________________________________
void laserInterface::startMonitors()
{
...
//                                        it2.second.MASK, laserInterface::staticEntryMagnetMonitor,
