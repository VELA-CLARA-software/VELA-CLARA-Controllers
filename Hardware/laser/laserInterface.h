...
class laserInterface : public interface
{
    public:
        ...
        laserInterface( const std::string &laserConf,
        ...
        ~laserInterface();
      //  laserInterface(const laserInterface& origin, const bool* show_messages_ptr, const bool * show_debug_messages_ptr); // add this line
    ...
    private:
        ...
        void killMonitor( laserStructs::monitorStruct * ms );

        void initialise();
        bool initObjects();
        void initChids();
        void addChannel( const std::string & pvRoot, laserStructs::pvStruct & pv );
        void startMonitors();

        //std::map< std::string, magnetStructs::magnetObject > allMagnetData;

        laserStructs::laserObject laser;

        std::vector< laserStructs::monitorStruct * > continuousMonitorStructs;

        laserConfigReader configReader; /// class member so we can pass in file path in ctor
        ///message
};
